...
def normalize_position_item(manifest, item):
    ...
    if "bezier_px" in item:
        curve = item["bezier_px"]
        points = [curve.get("start")] + [point for segment in curve.get("segments", []) for point in (segment.get("c1"), segment.get("c2"), segment.get("end"))]
        points = [(float(point[0]), float(point[1])) for point in points if point and len(point) == 2]
        if points and "box_px" not in item:
            xs = [point[0] for point in points]; ys = [point[1] for point in points]
            item["box_px"] = [min(xs), min(ys), max(max(xs) - min(xs), 1.0), max(max(ys) - min(ys), 1.0)]
...
LINE_END_TYPES = {"none", "triangle", "stealth", "diamond", "oval", "arrow"}
LINE_END_SIZES = {"sm", "med", "lg"}


def line_end_xml(tag, marker, width=None, length=None):
    marker = str(marker or "none").strip()
    if marker not in LINE_END_TYPES:
        raise ValueError(f"unsupported line-end marker: {marker}")
    if marker == "none":
        return ""
    attrs = [f'type="{marker}"']
    if width:
        width = str(width).strip()
        if width not in LINE_END_SIZES:
            raise ValueError(f"unsupported line-end width: {width}")
        attrs.append(f'w="{width}"')
    if length:
        length = str(length).strip()
        if length not in LINE_END_SIZES:
            raise ValueError(f"unsupported line-end length: {length}")
        attrs.append(f'len="{length}"')
    return f'<a:{tag} {" ".join(attrs)}/>'


def shape_line_xml(stroke, width, dash=None, start_arrow=None, end_arrow=None, start_arrow_width=None, start_arrow_length=None, end_arrow_width=None, end_arrow_length=None):
    if not stroke or stroke == "none":
        return '<a:ln><a:noFill/></a:ln>'
    dash_xml = f'<a:prstDash val="{xml_text(dash)}"/>' if dash else ""
    head_xml = line_end_xml("headEnd", start_arrow, start_arrow_width, start_arrow_length)
    tail_xml = line_end_xml("tailEnd", end_arrow, end_arrow_width, end_arrow_length)
    return (
...
def shape_xml(idx, item):
    ...
    line = shape_line_xml(
        ...
        item.get("end_arrow"),
        item.get("start_arrow_width"),
        item.get("start_arrow_length"),
        item.get("end_arrow_width"),
        item.get("end_arrow_length"),
    )
...
def custom_bezier_geometry_xml(item):
    curve = item.get("bezier_px", {})
    start = curve.get("start")
    segments_data = curve.get("segments", [])
    if not start or not segments_data:
        raise ValueError("curve shape requires bezier_px.start and at least one segment")
    left, top, width, height = [float(value) for value in item.get("box_px", [0, 0, 1, 1])]
    width = max(width, 1.0); height = max(height, 1.0)

    def rel(point):
        return int(round((float(point[0]) - left) / width * 21600)), int(round((float(point[1]) - top) / height * 21600))

    sx, sy = rel(start)
    commands = [f'<a:moveTo><a:pt x="{sx}" y="{sy}"/></a:moveTo>']
    for index, segment in enumerate(segments_data):
        if not all(segment.get(key) and len(segment[key]) == 2 for key in ("c1", "c2", "end")):
            raise ValueError(f"bezier_px.segments[{index}] requires c1, c2, and end")
        c1x, c1y = rel(segment["c1"]); c2x, c2y = rel(segment["c2"]); ex, ey = rel(segment["end"])
        commands.append(f'<a:cubicBezTo><a:pt x="{c1x}" y="{c1y}"/><a:pt x="{c2x}" y="{c2y}"/><a:pt x="{ex}" y="{ey}"/></a:cubicBezTo>')
    if curve.get("closed"):
        commands.append("<a:close/>")
    return ('<a:custGeom><a:avLst/><a:gdLst/><a:ahLst/><a:cxnLst/><a:rect l="l" t="t" r="r" b="b"/>'
            '<a:pathLst><a:path w="21600" h="21600">' + "".join(commands) + '</a:path></a:pathLst></a:custGeom>')
...
def slide_size_type(width, height):
    # ST_SlideSizeType uses the OOXML token ``screen16x9``.  ``wide`` is an
    # Office UI label, not a schema value; PowerPoint repairs files that use it.
  ...
  <a:themeElements>
    ...
    <a:fmtScheme name="Office">
      <a:fillStyleLst>
        ...
        <a:solidFill><a:schemeClr val="phClr"><a:shade val="50000"/><a:satMod val="200000"/></a:schemeClr></a:solidFill>
      </a:fillStyleLst>
      <a:lnStyleLst>
...
def render_preview(manifest, manifest_path, out_path):
    ...
    def draw_line_marker(point, other, marker, color, stroke_width):
        marker = str(marker or "none")
        if marker == "none" or color in (None, "none"):
            return
        px, py = point
        ox, oy = other
        dx, dy = px - ox, py - oy
        ...
        nx, ny = -uy, ux
        size = max(5.0, float(stroke_width) * 4.0)
        back = (px - ux * size, py - uy * size)
        half = size * 0.48
        left = (back[0] + nx * half, back[1] + ny * half)
        right = (back[0] - nx * half, back[1] - ny * half)
        if marker in {"triangle", "stealth"}:
            draw.polygon([point, left, right], fill=color)
        elif marker == "arrow":
            draw.line([left, point, right], fill=color, width=max(1, int(stroke_width)))
        elif marker == "diamond":
            far = (px - ux * size * 1.8, py - uy * size * 1.8)
            draw.polygon([point, left, far, right], fill=color)
        elif marker == "oval":
            radius = size * 0.45
            center = (px - ux * radius, py - uy * radius)
            draw.ellipse([center[0] - radius, center[1] - radius, center[0] + radius, center[1] + radius], fill=color)

    def open_preview_image(src):
        if src.suffix.lower() not in {".svg", ".emf"}:
            return Image.open(src).convert("RGBA")
        # Windows ships an unrelated disk utility named convert.exe.  Only
        # accept the legacy ImageMagick `convert` command on non-Windows hosts.
        convert = shutil.which("magick")
        if not convert and sys.platform != "win32":
            convert = shutil.which("convert")
        if not convert:
            print(f"Warning: cannot preview vector image without ImageMagick: {src}", file=sys.stderr)
            return None
        # NamedTemporaryFile remains locked on Windows and ImageMagick cannot
        # overwrite it. Use a private temporary directory with a not-yet-open
        # output path, then fully load/copy the pixels before cleanup.
        with tempfile.TemporaryDirectory() as directory:
            preview_path = Path(directory) / "vector-preview.png"
            subprocess.run([convert, str(src), str(preview_path)], check=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
            with Image.open(preview_path) as preview:
                return preview.convert("RGBA").copy()
    ...
    def render_shape(item):
        ...
        if item.get("bezier_px"):
            ...
            points_px = sampled_curve(item["bezier_px"], samples_per_segment=32)
            points = []
            for x, y in points_px:
                placed = px_to_inches(manifest, x, y, 0, 0)
                points.append((placed["left"] * scale, placed["top"] * scale))
            if item["bezier_px"].get("closed") and len(points) > 2 and fill not in (None, "none"):
                draw.polygon(points, fill=fill)
            if outline not in (None, "none"):
                draw.line(points, fill=outline, width=width, joint="curve")
                if item["bezier_px"].get("closed") and len(points) > 2:
            ...
            if "points" in item:
                ...
                draw_line_marker(start, end, item.get("start_arrow"), outline, width)
                draw_line_marker(end, start, item.get("end_arrow"), outline, width)
                return
            if item.get("dash"):
