...
def cmd_page_validate(args: argparse.Namespace) -> int:
    ...
    if args.source:
        argv.extend(["--source", str(page_dir / args.source)])
    if args.preview:
        argv.extend(["--preview", str(page_dir / args.preview)])
    if args.report:
...
def cmd_curve_fit(args: argparse.Namespace) -> int:
    argv = [args.page_dir, "--points-file", args.points_file, "--out", args.out, "--id", args.id, "--stroke", args.stroke, "--stroke-width", str(args.stroke_width), "--tolerance", str(args.tolerance), "--tension", str(args.tension)]
    if args.closed: argv.append("--closed")
    return run_script("bezier_curve.py", argv)


def cmd_curve_trace(args: argparse.Namespace) -> int:
    argv = [args.page_dir, "--source", args.source, "--roi", args.roi, "--out", args.out, "--id", args.id, "--target-color", args.target_color, "--color-distance", str(args.color_distance), "--max-gap", str(args.max_gap), "--max-step-y", str(args.max_step_y), "--line-style", args.line_style, "--dash-max-component-width", str(args.dash_max_component_width), "--axis-clearance", args.axis_clearance, "--fit-tolerance", str(args.fit_tolerance), "--stroke", args.stroke, "--stroke-width", str(args.stroke_width)]
    if args.preview: argv.extend(["--preview", args.preview])
    if args.dash: argv.extend(["--dash", args.dash])
    return run_script("trace_plot_curve.py", argv)


def cmd_vectorize_icon(args: argparse.Namespace) -> int:
    argv = [args.page_dir, "--input", args.input, "--out", args.out, "--colors", str(args.colors), "--tolerance", str(args.tolerance), "--id", args.id]
    for flag, value in (("--emf", args.emf), ("--fragment", args.fragment), ("--box", args.box)):
        if value: argv.extend([flag, value])
    return run_script("vectorize_icon.py", argv)


def cmd_layout_optimize(args: argparse.Namespace) -> int:
    return run_script("layout_optimizer.py", [args.page_dir, "--manifest", args.manifest, "--report", args.report, "--iterations", str(args.iterations)])
...
def build_parser() -> argparse.ArgumentParser:
  ...
  - curve traces source pixels or fits sampled paths to editable cubic Bezier geometry.
  - vectorize converts separated icon PNGs to SVG and optional EMF.
  - layout applies global alignment, containment, spacing, and non-overlap constraints.
...
Examples:
  ...
  editppt curve fit pages/page_001 --points-file curve.json --out curve-fragment.json
  editppt curve trace pages/page_001 --roi 620,160,380,120 --out assets/main-curve.json --preview assets/main-curve-trace.png --id main_curve
  editppt vectorize icon pages/page_001 --input assets/icons/i1.png --out assets/icons/i1.svg --box 10,20,60,60 --fragment icon.json
  editppt layout optimize pages/page_001
...
""",
    ...
    curve = sub.add_parser("curve", help="Trace source plots or fit sampled curves to editable cubic Bezier paths.")
    curve_sub = curve.add_subparsers(dest="curve_command", required=True)
    curve_fit = curve_sub.add_parser("fit", help="Fit a points JSON file to bezier_px geometry.")
    curve_fit.add_argument("page_dir"); curve_fit.add_argument("--points-file", required=True); curve_fit.add_argument("--out", required=True); curve_fit.add_argument("--id", default="curve"); curve_fit.add_argument("--stroke", default="#000000"); curve_fit.add_argument("--stroke-width", type=float, default=1.0); curve_fit.add_argument("--tolerance", type=float, default=1.5); curve_fit.add_argument("--tension", type=float, default=1.0); curve_fit.add_argument("--closed", action="store_true"); curve_fit.set_defaults(func=cmd_curve_fit)
    curve_trace = curve_sub.add_parser("trace", help="Trace a plotted source-image stroke before cubic fitting.")
    curve_trace.add_argument("page_dir"); curve_trace.add_argument("--source", default="source.png"); curve_trace.add_argument("--roi", required=True); curve_trace.add_argument("--out", required=True); curve_trace.add_argument("--preview"); curve_trace.add_argument("--id", required=True); curve_trace.add_argument("--target-color", default="#0B3B99"); curve_trace.add_argument("--color-distance", type=float, default=48.0); curve_trace.add_argument("--max-gap", type=int, default=18); curve_trace.add_argument("--max-step-y", type=float, default=8.0); curve_trace.add_argument("--line-style", choices=["auto", "solid", "dashed"], default="auto"); curve_trace.add_argument("--dash-max-component-width", type=int, default=32); curve_trace.add_argument("--axis-clearance", default="2,1,2,3"); curve_trace.add_argument("--fit-tolerance", type=float, default=0.8); curve_trace.add_argument("--stroke", default="#0B3B99"); curve_trace.add_argument("--stroke-width", type=float, default=1.2); curve_trace.add_argument("--dash"); curve_trace.set_defaults(func=cmd_curve_trace)

    vectorize = sub.add_parser("vectorize", help="Vectorize separated foreground assets.")
    vector_sub = vectorize.add_subparsers(dest="vector_command", required=True)
    vector_icon = vector_sub.add_parser("icon", help="Convert a separated transparent PNG icon to SVG/EMF.")
    vector_icon.add_argument("page_dir"); vector_icon.add_argument("--input", required=True); vector_icon.add_argument("--out", required=True); vector_icon.add_argument("--emf"); vector_icon.add_argument("--colors", type=int, default=8); vector_icon.add_argument("--tolerance", type=float, default=1.2); vector_icon.add_argument("--fragment"); vector_icon.add_argument("--id", default="vector_icon"); vector_icon.add_argument("--box"); vector_icon.set_defaults(func=cmd_vectorize_icon)

    layout = sub.add_parser("layout", help="Apply global source-pixel layout constraints.")
    layout_sub = layout.add_subparsers(dest="layout_command", required=True)
    layout_opt = layout_sub.add_parser("optimize", help="Optimize manifest global_layout constraints in place.")
    layout_opt.add_argument("page_dir"); layout_opt.add_argument("--manifest", default="manifest.json"); layout_opt.add_argument("--report", default="layout-optimization.json"); layout_opt.add_argument("--iterations", type=int, default=12); layout_opt.set_defaults(func=cmd_layout_optimize)
...
""",
    ...
    page_validate.add_argument("--source", default="source.png", metavar="FILE", help="Source image used for object-level geometry comparison.")
    page_validate.add_argument("--preview", default="preview.png", metavar="FILE", help="Rendered preview used for object-level geometry comparison.")
