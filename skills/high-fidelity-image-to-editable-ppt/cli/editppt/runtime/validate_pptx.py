...
REQUIRED_QUALITY_CHECKS = {
    ...
    "bezier_curves_checked",
    "vector_assets_checked",
    "global_layout_optimized",
    "curve_source_traced",
    "axis_clearance_checked",
    "decorative_assets_checked",
    "geometry_inventory_checked",
    "structural_fidelity_checked",
...
def quality_contract_violations(manifest):
        ...
        if shape.get("type") == "line":
            for key in ("start_arrow", "end_arrow"):
                ...
                if marker not in ALLOWED_MARKERS:
                    violations.append({"field": f"shapes[{index}].{key}", "reason": f"line-end marker must be one of {sorted(ALLOWED_MARKERS)}"})
        if shape.get("type") == "curve" or shape.get("bezier_px"):
            curve = shape.get("bezier_px")
            if not isinstance(curve, dict) or not curve.get("start") or not curve.get("segments"):
                violations.append({"field": f"shapes[{index}].bezier_px", "reason": "curve requires a start point and cubic segments"})
            else:
                for segment_index, segment in enumerate(curve.get("segments", [])):
                    if not isinstance(segment, dict) or any(not segment.get(key) or len(segment[key]) != 2 for key in ("c1", "c2", "end")):
                        violations.append({"field": f"shapes[{index}].bezier_px.segments[{segment_index}]", "reason": "each cubic segment requires c1, c2, and end pixel points"})
            ...
            if role == "data-stroke":
                ...
                if not plot or not isinstance(clearance, dict):
                    violations.append({"field": f"shapes[{index}].plot_area_px/axis_clearance_px", "reason": "data curves require plot bounds and explicit axis clearances"})
                elif curve and curve.get("segments"):
                    left, top, width, height = [float(value) for value in plot]
                    limits = (left + float(clearance.get("left", 0)), top + float(clearance.get("top", 0)), left + width - float(clearance.get("right", 0)), top + height - float(clearance.get("bottom", 0)))
                    for x, y in sampled_curve(curve, 64):
                        if x < limits[0] or y < limits[1] or x > limits[2] or y > limits[3]:
                            violations.append({"field": f"shapes[{index}].axis_clearance_px", "reason": "curve geometry enters the protected coordinate-axis clearance"})
                            break
        if shape.get("preset") in COMPLEX_ARROW_PRESETS:
            role = shape.get("source_role")
            if role != "native-structural":
                violations.append({"field": f"shapes[{index}].source_role", "reason": "complex/decorative curved arrows must use source-faithful asset-sheet separation; only explicitly native-structural arrows may use a preset"})
            elif not shape.get("geometry_comparison_note"):
                violations.append({"field": f"shapes[{index}].geometry_comparison_note", "reason": "native-structural complex arrows require a source geometry comparison note"})
    ...
    layout = manifest.get("global_layout")
    if layout is not None:
        if not isinstance(layout, dict) or not isinstance(layout.get("constraints", []), list):
            violations.append({"field": "global_layout", "reason": "global_layout must be an object with a constraints list"})
        ids = [str(item.get("id")) for section in ("shapes", "images", "text_boxes") for item in manifest.get(section, []) if item.get("id")]
        if len(ids) != len(set(ids)):
            violations.append({"field": "global_layout", "reason": "layout participant ids must be unique"})
        unresolved = manifest.get("layout_optimization", {}).get("unresolved_overlaps", [])
        if unresolved:
            violations.append({"field": "layout_optimization.unresolved_overlaps", "reason": "global layout has unresolved overlaps"})

    for index, image in enumerate(manifest.get("images", [])):
        suffix = Path(image.get("path", "")).suffix.lower()
        if suffix in {".svg", ".emf"} and image.get("vectorization"):
...
def pixel_authoring_violations(manifest):
    ...
    for index, item in enumerate(manifest.get("shapes", [])):
        if item.get("type") == "curve" or item.get("bezier_px"):
            if "bezier_px" not in item:
                violations.append({"field": f"shapes[{index}].bezier_px", "reason": "curve shapes must use source-image cubic Bezier coordinates"})
...
def office_schema_violations(z, names):
    """Check core OOXML invariants that make PowerPoint repair a package.

    This intentionally complements, rather than pretends to replace, a full
    Open XML SDK schema pass.  These package parts are authored by this runtime,
    so their fragile required values and list cardinalities are deterministic.
    """
    violations = []
    presentation_part = "ppt/presentation.xml"
    if presentation_part in names:
        try:
            presentation = ET.fromstring(z.read(presentation_part))
            slide_size = presentation.find("p:sldSz", NS)
            size_type = slide_size.attrib.get("type") if slide_size is not None else None
            if size_type and size_type not in VALID_SLIDE_SIZE_TYPES:
                violations.append({
                    "part": presentation_part,
                    "path": "/p:presentation/p:sldSz/@type",
                    "reason": f"invalid ST_SlideSizeType value: {size_type}",
                })
        except Exception as exc:
            violations.append({"part": presentation_part, "reason": f"invalid XML: {exc}"})

    for theme_part in sorted(name for name in names if re.match(r"ppt/theme/theme\d+\.xml$", name)):
        try:
            theme = ET.fromstring(z.read(theme_part))
            for fmt_index, fmt_scheme in enumerate(theme.findall(".//a:fmtScheme", NS), start=1):
                for list_name in ("fillStyleLst", "lnStyleLst", "effectStyleLst", "bgFillStyleLst"):
                    style_list = fmt_scheme.find(f"a:{list_name}", NS)
                    count = len(list(style_list)) if style_list is not None else 0
                    if count < 3:
                        violations.append({
                            "part": theme_part,
                            "path": f"/a:theme/a:themeElements/a:fmtScheme[{fmt_index}]/a:{list_name}",
                            "reason": f"incomplete theme style list: expected at least 3 entries, found {count}",
                        })
        except Exception as exc:
            violations.append({"part": theme_part, "reason": f"invalid XML: {exc}"})
    return violations
...
def validate_deck(args):
    ...
    try:
        with zipfile.ZipFile(args.pptx) as z:
            ...
            report["office_schema_violations"] = office_schema_violations(z, names)
    ...
    report["passed"] = (
        ...
        and not report["office_schema_violations"]
...
def main():
    ...
    source_path = Path(args.source) if args.source else Path(raw_manifest.get("source", {}).get("path", "source.png"))
    preview_path = Path(args.preview) if args.preview else Path("preview.png")
    if not source_path.is_absolute():
        source_path = manifest_base / source_path
