...
|   `-- validate                - validate structure plus object-level source/render geometry as run record will
|-- curve fit                   - fit sampled points to editable cubic Bezier geometry
|-- curve trace                 - trace a plotted stroke from source pixels, then fit cubic geometry
|-- vectorize icon              - trace a separated icon PNG to SVG/optional EMF
|-- layout optimize             - apply global layout constraints and report conflicts
...
Advanced reconstruction commands run before `page build`:

```bash
editppt curve fit pages/page_001 --points-file assets/curve-points.json --out assets/curve.fragment.json --id trend_curve --stroke "#1246A0" --stroke-width 2 --tolerance 1.5
editppt curve trace pages/page_001 --roi 625,164,375,116 --target-color "#0B3B99" --axis-clearance 3,1,3,4 --out assets/main-curve.fragment.json --preview assets/main-curve-trace.png --id main_curve
editppt vectorize icon pages/page_001 --input assets/icons/icon_01.png --out assets/icons/icon_01.svg --emf assets/icons/icon_01.emf --colors 8 --tolerance 1.2 --box 100,100,64,64 --fragment assets/icons/icon_01.fragment.json
editppt layout optimize pages/page_001 --manifest manifest.json --report layout-optimization.json
```

`vectorize icon` accepts only an icon already produced by the asset-sheet separation workflow. EMF is optional and requires Inkscape; SVG remains authoritative when it is unavailable. Run layout optimization after all objects and constraints are present.

Use `curve trace`, not `curve fit`, for chart/data curves. Keep the ROI inside the plot, inspect `--preview`, and merge the emitted shape into the manifest. `--axis-clearance left,top,right,bottom` creates a hard protected boundary that validation rechecks after fitting.
