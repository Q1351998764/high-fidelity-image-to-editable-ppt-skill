...
Record the inventory in `visual_inventory` and `geometry_inventory`, and record the decisions of the next three sections in `background_strategy` and `quality_checks` (field contracts in `manifest-schema.md`; every required `quality_checks` flag must end up `true`).
...
- Decorative red/colored curved arrows and complex arrow components whose source silhouette differs from a standard PowerPoint arrow preset.
...
After a source-faithful asset sheet has been generated, de-keyed, and split, `editppt vectorize icon` may deterministically trace a separated PNG to SVG and optionally EMF. This is only a post-separation representation change and never a substitute for separation or permission to trace a direct source crop.
...
For every axis, inspect both endpoints at source-pixel zoom and record `expected_start_marker` and `expected_end_marker` even when either value is `none`. Use real DrawingML line ends (`start_arrow` / `end_arrow`) for triangle, stealth, arrow, diamond, or oval markers; never substitute an unmarked line when the source has an arrowhead.

Classify every bracket-like component before choosing its representation: `square`, `round`, `curly`, or `measurement`, plus `horizontal` or `vertical`. Use the matching editable PowerPoint bracket/brace/parenthesis preset when its silhouette matches; otherwise use a source-traced editable curve. A curly brace assembled from straight lines is a page failure.

Keep the visible data stroke open with `fill: none`. If the source has under-curve shading, create a separate closed `area-fill` path; never close the data stroke to the baseline because this changes its endpoint geometry. Record `plot_area_px`, `axis_clearance_px`, and source trace points. The fitted curve must stay inside the protected plot interior: it may approach an axis exactly as the source does, but it must not touch or overlap an axis when the source does not. Source-to-render Chamfer error above the manifest limit is a page failure.
...
- Decorative curved arrows retain their source silhouette through asset-sheet separation instead of a mismatched PowerPoint preset.
- Icon rows remain clear of the plot region and do not overlap the last curve row.
- Vectorized icons were traced only after asset-sheet separation and contain no embedded raster payload.
- `layout-optimization.json` reports no unresolved overlaps.
