...
```text
Rebuild one page for high-fidelity-image-to-editable-ppt.
...
5. Write manifest.json following the field contracts in manifest-schema.md, including `text_inventory`, `visual_inventory`, `geometry_inventory`, `background_strategy`, `quality_checks`, and positioned `text_boxes`/`images`/`shapes`. Inventory every axis endpoint marker, bracket family/orientation, legend curve/baseline relationship, and small structural curve with a tight source ROI. Trace every chart/data curve and legend curve from source pixels with `editppt curve trace`, inspect each trace preview, keep data strokes open and their under-curve fills separate, and record axis clearances. Use real `start_arrow`/`end_arrow` DrawingML markers for source axes, and matching editable bracket/brace presets or source-traced curves for brackets. Use `curve fit` only for known non-data points. Treat decorative/complex arrows as foreground assets unless they are explicitly source-matched native structural arrows. After compliant asset-sheet separation, vectorize suitable icons with `editppt vectorize icon`; encode cross-object relationships under `global_layout`, including `avoid` constraints between plots and icon rows.
