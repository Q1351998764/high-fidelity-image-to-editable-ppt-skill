...
6. The parent runtime must have passed the OCR quality gate before this page is dispatched. If `text_hints.json.backend` is not `paddleocr-vl`, verify that `deck_manifest.json.text_hints_policy.mode` is `offline-authorized`; otherwise stop and report an invalid dispatch instead of silently using offline hints.
...
5. Write manifest.json following the field contracts in manifest-schema.md, including `text_inventory`, `visual_inventory`, `geometry_inventory`, `micro_annotation_inventory` for plotted pages, `background_strategy`, `quality_checks`, and positioned `text_boxes`/`images`/`shapes`. Map every positioned image to a structured visual-inventory object id, explain every repeated asset path with `reuse_reason`, and give every visible line/curve a `semantic_role`. Inventory every plot-side `t`, `%`, tick/unit label, and tiny arrowhead separately; use real line-end arrows rather than combining `t →` into one text glyph. Geometry inventory cannot be empty when lines/curves exist; inventory every axis endpoint marker, bracket family/orientation, legend curve/baseline relationship, and native structural curve with a tight source ROI. Trace every non-fill curve from source pixels, retain `source_roi_px`, and verify the trace covers the source stroke rather than one short segment. Keep data strokes open and their under-curve fills separate, and record axis clearances. Preserve broad palette/chroma semantics for multicolor assets and the ordered filled/empty sequence of small state cells; exact RGB equality is not required. Use real `start_arrow`/`end_arrow` DrawingML markers for source axes, matching editable bracket/brace presets or source-traced curves for brackets, and `curve fit` only for already source-traced non-data points. Treat decorative/complex arrows as foreground assets unless they are explicitly source-matched native structural arrows. After compliant asset-sheet separation, vectorize suitable icons with `editppt vectorize icon`; encode cross-object relationships under `global_layout`, including `avoid` constraints between plots and icon rows. Resolve every material text-box overlap or explicitly declare the source-intentional pair with `allow_overlap_with`.
6. Run `editppt layout optimize {{PAGE_DIR}}` and resolve every overlap, axis-clearance entry, and icon/plot collision, then build the artifacts with the deterministic runtime: `editppt page build {{PAGE_DIR}}` (writes page.pptx and preview.png from manifest.json), then `editppt page contact-sheet {{PAGE_DIR}}`, then `editppt page validate {{PAGE_DIR}}` — it runs the same manifest-contract checks `editppt run record` will run, so fix every reported issue here, inside the page.
...
Before returning, run the Final Self-Check in page-decision-tree.md once: compare preview.png and split_assets_contact.png to the source, resolve every reverse-coverage hotspot, confirm micro labels/arrows are present, all curves cover their source strokes, axis endpoint markers match, bracket families/orientations match, multicolor assets retain broad palette semantics, repeated state cells preserve their filled/empty sequence, decorative arrows retain the source style, icon rows do not overlap plots, vector icons contain no embedded raster, every object-level geometry edge check passes, layout optimization has zero unresolved overlap, `editppt page validate {{PAGE_DIR}}` passes, validation.json contains top-level `passed: true`, and all required outputs exist. Page-local issues are fixed inside the current page by you before returning.
