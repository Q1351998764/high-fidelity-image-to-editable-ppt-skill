...
5. Write manifest.json following the field contracts in manifest-schema.md, including `text_inventory`, `visual_inventory`, `geometry_inventory`, `background_strategy`, `quality_checks`, and positioned `text_boxes`/`images`/`shapes`. Map every positioned image to a structured visual-inventory object id, explain every repeated asset path with `reuse_reason`, and give every visible line/curve a `semantic_role`. Geometry inventory cannot be empty when lines/curves exist; inventory every axis endpoint marker, bracket family/orientation, legend curve/baseline relationship, and native structural curve with a tight source ROI. Trace every non-fill curve from source pixels, including chart/data, legend, and native structural curves; inspect each trace preview, keep data strokes open and their under-curve fills separate, and record axis clearances. Use real `start_arrow`/`end_arrow` DrawingML markers for source axes, and matching editable bracket/brace presets or source-traced curves for brackets. Use `curve fit` only for already source-traced non-data points. Treat decorative/complex arrows as foreground assets unless they are explicitly source-matched native structural arrows. After compliant asset-sheet separation, vectorize suitable icons with `editppt vectorize icon`; encode cross-object relationships under `global_layout`, including `avoid` constraints between plots and icon rows. Resolve every material text-box overlap or explicitly declare the source-intentional pair with `allow_overlap_with`.
...
Before returning, run the Final Self-Check in page-decision-tree.md once: compare preview.png and split_assets_contact.png to the source, confirm data and legend curves follow their pixel traces, axis endpoint markers match, bracket families/orientations match, declared curve-to-baseline relationships hold, decorative arrows retain the source style, icon rows do not overlap plots, vector icons contain no embedded raster, every object-level geometry edge check passes, layout optimization has zero unresolved overlap, `editppt page validate {{PAGE_DIR}}` passes, validation.json contains top-level `passed: true`, and all required outputs exist. Page-local issues are fixed inside the current page by you before returning.
