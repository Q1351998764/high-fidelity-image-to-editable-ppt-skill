---
name: high-fidelity-image-to-editable-ppt
description: Rebuild slide images, image-based or scanned PPT/PPTX files, and PDF decks into high-fidelity object-level editable PowerPoint (.pptx), with source-traced Bézier curves, vector assets, geometry-aware visual comparison, layout constraints, and OOXML validation. Use whenever the user provides a visual slide source and wants an editable reconstruction — "make this PPT editable", "把图片/截图转成高保真可编辑 PPT", "restore this scanned deck", or equivalent requests. Not for authoring new presentations from scratch.
---
# High-Fidelity Image to Editable PPT

## Overview

This skill rebuilds visual slide inputs into object-level editable PowerPoint `.pptx` files.

The deterministic runtime traces chart and legend strokes from source pixels before cubic Bézier fitting, verifies trace coverage against the actual source-color stroke, preserves source line-end markers, validates plot-side micro annotations, performs whole-page reverse source coverage, checks tolerant Lab color semantics for icons/state cells, validates bracket families and coordinate-axis clearance, vectorizes already separated icon assets to SVG/EMF, and applies cross-object collision constraints before build. These additions do not weaken the required source-faithful asset-sheet workflow.
...
- Page validation is source-open-world rather than manifest-closed-world: whole-page reverse coverage may reject a source object that was omitted from every inventory. Plot pages must inventory micro labels/arrows, curve traces must cover their source ROI, and color checks allow mild drift while rejecting grayscale collapse or changed state-cell counts.
- Fidelity gates are page-adaptive: pixel distances and micro-object sizes scale with the source page, omissions require agreement across resolutions and evidence types, curve completeness is measured against the source-colored stroke actually present, palette groups are clustered in Lab space, and repeated state cells are inferred from geometry rather than English ids. Manifest fields cannot relax these derived hard limits.
...
- `editppt run next` is an OCR quality gate as well as an orchestrator. It must not dispatch or locally rebuild any page whose `text_hints.json.backend` is not `paddleocr-vl`. If PaddleOCR is unavailable, continue only after the user explicitly approves offline-only hints and record that approval with `editppt run allow-offline-hints <run> --reason "..."`; an agent may never invent this authorization.
...
Prepare also writes per-page text hints. Whenever `editppt doctor`, prepare, or `editppt run next` reports that no PaddleOCR token is configured, ask the user before dispatching any page: a token from https://aistudio.baidu.com/account/accessToken stored via `editppt config --paddle-ocr-token <token>` makes the hints content-aware and noticeably improves text fidelity, and `editppt run hints <run>` regenerates the current run's hints in place. Wait for their choice. If they explicitly decline, request local-only processing, or knowingly accept the quality loss, persist that decision with `editppt run allow-offline-hints <run> --reason "<user-approved reason>"`; without that persisted authorization, `run next` must remain blocked.

If a PaddleOCR token is already configured but `prepare` falls back because network access, DNS, sandbox approval, service failure, or an unusably sparse OCR result, request network approval with the justification described in the Entry Contract and rerun `editppt run hints <run>` before page reconstruction. The fallback is never silent: `run next` reports `ocr_quality_gate` and refuses dispatch. If the approved retry still fails, explain the concrete failure and ask the user for explicit authorization before recording an offline override.
