...
- 对图表周边的 `t`、`%`、刻度、单位和微型箭头按页面尺度执行多尺度漏检检查。
- 同时验证“贝塞尔—Trace”和“Trace—源图实际色彩笔画”；短局部曲线可以通过，只追踪长曲线的一小段会失败。
- 使用自适应 Lab 调色板聚类和局部色彩离散度检查；允许轻微色偏，但拒绝彩色组件变黑白或状态格数量改变。
- 通过相似尺寸、对齐和间距识别重复状态格，不依赖对象英文 ID，也不允许 Manifest 自行放宽硬门禁。
...
7. **曲线和矢量化**：所有非填充曲线（数据、图例和结构曲线）都必须从源像素追踪并拟合为可编辑三次贝塞尔路径；验证 Trace 相对源图中实际色彩笔画的跨度、点支持和覆盖率，不要求占满固定比例的 ROI；合规的分离图标可进一步转为 SVG/EMF。
8. **全局排版优化**：执行包含、对齐、等距、流式排列、非重叠和图标避让约束，并对未声明关系的文本框执行全局碰撞检测。
9. **开放世界保真校验**：除了检查 Manifest 已声明对象，还在全分辨率与降采样视图中反向查找预览无法解释的源图边缘；像素距离、微标注尺寸和热点规模随页面自适应，只有多种证据一致时才硬失败。图表必须登记微型文字和箭头，`geometry_inventory` 检查轴线、括号、图例和结构曲线。
10. **容差型颜色语义校验**：图标在 Lab 色彩空间自适应聚类，重复状态格由几何阵列识别，并根据各格源图内部的颜色离散度生成容差。压缩、抗锯齿、亮度和轻微色偏可通过，彩色变灰及填充/空白状态改变会失败。
