...
7. **曲线和矢量化**：所有非填充曲线（数据、图例和结构曲线）都必须从源像素追踪并拟合为可编辑三次贝塞尔路径；合规的分离图标可进一步转为 SVG/EMF。
8. **全局排版优化**：执行包含、对齐、等距、流式排列、非重叠和图标避让约束，并对未声明关系的文本框执行全局碰撞检测。
9. **几何与素材保真校验**：`geometry_inventory` 不得在存在线条/曲线时为空，并检查轴线箭头、括号类型与方向、图例比例、结构曲线和曲线—基线关系；每个图片对象必须被 `visual_inventory` 逐一覆盖，重复素材必须说明原因。
